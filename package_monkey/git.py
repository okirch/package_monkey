...
class Git(object):
	...
	def getLocalPathFor(self, name):
		return f"{self.workPath}/{name}"

	def clone(self, gitUrl, repositoryLocalPath = None, name = None):
		if repositoryLocalPath is None:
			if name is None:
				raise Exception(f"Unable to clone {gitUrl}: you need to specify either name or repositoryLocalPath")
			repositoryLocalPath = self.getLocalPathFor(name)
		...
		work = GitWorkingCopy(self.client, repositoryLocalPath)
		work.clone(gitUrl)
		return work
...
class GitWorkingCopy(GitObject):
	class Change(object):
		def __init__(self, verb, type, name, cleanup = None):
			self.verb = verb
			self.type = type
			self.name = name
	...
	def getRevisionHash(self, tag = None):
		command = "git show -s --format=%H"
		if tag is not None:
			command += ' ' + tag

		with self.popen(command) as f:
			content = list(f.readlines())

		if not content:
			return None
		if len(content) > 1:
			raise Exception(f"Ambiguous content from {command}: {content}")
	...
	def addFile(self, name, content):
		...
		if os.path.exists(path):
			if not os.path.isfile(path):
				raise Exception(f"Cannot update file {name}: already exists and is not a regular file")
			fileExisted = True

			current = open(path, "r").read()
			if current == content:
				infomsg(f"   {name} is unchanged")
				return

		infomsg(f"Updating file {name} in git working copy")
		...
		if somethingChanged:
			...
			if fileExisted:
				verb = 'update'
			self._changes.append(self.Change(verb, 'package', name))
	...
	def removePackage(self, name):
		sm = self.getSubmodule(name)
		if sm is not None:
			self.run(f"git submodule deinit {name}")
		...
		if os.path.isdir(f"{self.path}/{name}"):
			...
			cleanupfn = None
			if sm is not None:
				cleanupfn = sm.cleanup

			self._changes.append(self.Change('remove', 'package', name, cleanup = cleanupfn))
	...
	def addSubmodule(self, gitUrl, name = None):
		...
		# cached?
		cachedPath = f"{self.path}/.git/modules/{name}"
		if os.path.isdir(cachedPath):
			infomsg(f"resurrect {name} from {cachedPath}")
			self.run(f"git submodule add --force {gitUrl}")
			self._changes.append(self.Change('re-add', 'submodule', name))
		else:
	...
	def updateSubmodules(self):
		def commitSubmoduleIfChanged(statusLine):
			w = change.rstrip().split()
			if len(w) != 2:
				return False
			...
			self._changes.append(self.Change('update', 'submodule', sm.name))
			return True

		self.run(f"git submodule foreach git update")
		# git status --porcelain --untracked-files=no
		changes = self.raw_status()

		nfailures = 0
		for change in changes:
			if not commitSubmoduleIfChanged(change):
				errormsg(f"Unable to process porcelain status line \"{change}\"")
				nfailures += 1
...
class GitSubmodule(GitWorkingCopy):
	...
	def commit(self):
		self.run(f"git commit -m\"Updated {self.name}\" {self.name}", path = self.parentPath)
		# self.run(f"git push", path = self.parentPath)

	# This gets invoked after a submodule has been deleted and the delete has been committed
	# It gets rid of the .git/modules/$name. If we do not do this, a later attempt to re-add
	# the module will result in funky error message and a corrupted state that is rather
	# hard to escape from
	def cleanup(self):
		cachePath = f".git/modules/{self.name}"
		if os.path.isdir(cachePath):
			infomsg(f"Deleting cached module {cachePath}")
			self.run(f"rm -rf {cachePath}", path = self.parentPath)
...
if __name__ == '__main__':
	...
	work = git.clone("gitea@src.suse.de:okir/sle16_foundations", name = "test")
