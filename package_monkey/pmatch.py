...
class ParallelStringMatcher:
	...
	class TableBuildCursor(object):
		def __init__(self, table):
			self.tables = [table]

		def add(self, cc):
			self.tables = list(t.add(cc) for t in self.tables)
		...
		def setSolution(self, match):
			for t in self.tables:
				t.solutions.append(match)

	def addToTable(self, table, string, match):
		cursor = self.TableBuildCursor(table)
		chars = list(string)
		...
		while chars:
			...
			while chars:
				...
				if cc != '-' or previous is None:
					charClass.add(cc)
					previous = cc
				else:
					cc = chars.pop(0)
					assert(cc != ']')
					if ord(cc) < ord(previous):
						raise Exception(f"bad match pattern {string}")
					charClass.update(map(chr, range(ord(previous), ord(cc))))
					previous = None

			cursor.addClass(charClass)

		cursor.setSolution(match)
